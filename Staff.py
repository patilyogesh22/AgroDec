...
#bg
bg=ImageTk.PhotoImage(file="Image\\teach.png")
l=Label(image=bg)
l.place(x=-180,y=-10)
...
def saveindb():
    ...
    query = "INSERT INTO faculty(Id,Name,Mobile,Address,Email,Adhar,Gender) VALUES(%s,%s,%s,%s,%s,%s,%s)"
    args = (fid,fname,fmobile,faddress,femail,fadhar,fgender)
    ...
    try:
        ...
        messagebox.showinfo("SUCCESSFULLY INSERTED", "Record of Faculty Inserted Successfully")
...
def updateindb():
    ...
    update="Update faculty set Name=%s,Mobile=%s,Address=%s,Email=%s,Adhar=%s,Gender=%s where Enroll=%s"
...
down=True
var=StringVar()
cal=cal(window,selectmode="day",year=2021,month=9,day=16)
...
#heading
H=Label(window,text="MEDICAPS TEACHER BUS SERVICE",height=0,width=0,font=("comic sans ms",25,"bold"))
H.config(bg="white",fg="black")
H.place(x=215,y=10)

#firstname
l1=Label(window,text="Enter Teacher ID",width=0,height=0,font=("comic sans ms",20,"bold"))
